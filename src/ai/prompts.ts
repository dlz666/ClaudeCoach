...
const INTERACTIVE_WIDGET_RULES = [
  ...
  '7. 主题色 **必须用 CSS 变量**：var(--bg) var(--fg) var(--accent) var(--accent-fg) var(--border) var(--input-bg) var(--input-fg) var(--muted) var(--panel-bg)。这些在 iframe :root 已注入，跟随 VS Code 主题',
  '8. 不要写死 width: 1000px 这种 → 用户侧栏可能很窄，组件要响应式',
  '9. **不要把整段 JS 包在 try/catch 里** —— 这会吞掉真实的逻辑 bug 让 widget 看起来"渲染成功但内容是空的"。让错误抛出，iframe bridge 的 error 监听器会显示醒目红色覆盖层，反而方便排查',
  '10. **写完代码自己脑中跑一遍**：',
  ...
  '11. **保持简单**：能用 200 行做出来就别写 500 行。多功能 ≠ 好 widget；可工作 + 教学清晰才是好 widget',
  '',
