...
const INTERACTIVE_WIDGET_RULES = [
  ...
  '4. JS 里如果有字符串包含 `</script>`，**一定写成 `<\\/script>`**，否则会提前关 script 标签把后面内容当成 HTML',
  '5. CSS 里如果字符串包含 `</style>`，写成 `<\\/style>`',
  '6. 主题色 **必须用 CSS 变量**：var(--bg) var(--fg) var(--accent) var(--accent-fg) var(--border) var(--input-bg) var(--input-fg) var(--muted) var(--panel-bg)。这些在 iframe :root 已注入，跟随 VS Code 主题',
  '7. 不要写死 width: 1000px 这种 → 用户侧栏可能很窄，组件要响应式',
  '8. JS 里 try/catch 重要操作，避免一处错误整个 widget 白屏',
  '9. 适合 200-500 行 HTML+JS；> 800 行说明可能不该用 widget',
  '',
  ...
  '  reset();',
  '} catch(e) {',
  '  document.body.insertAdjacentHTML("afterbegin", `<div style="color:red;padding:8px;border:1px solid red">widget error: ${e.message}<\\/div>`);',
  '}',
  '</script>',
  '```',
  '',
  '注意上例：SVG 写了完整 width/height/viewBox 三件套；JS 里 `</text>` `</g>` `</tr>` 等闭合都写成 `<\\/...>` 防止字符串被提前截断；try/catch 包住整段防白屏。**生成 widget 时严格照这个模板**。',
].join('\n');
