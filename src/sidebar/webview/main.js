(function () {
  ...
  function reconcileOutlineRebuildSelectedMaterials() {
    const allowedIds = new Set(getOutlineRebuildAvailableMaterials().map((material) => material.id));
    state.rebuildModal.selectedMaterialIds = (state.rebuildModal.selectedMaterialIds || []).filter((materialId) => allowedIds.has(materialId));
  }
  ...
  function openOutlineRebuildModal() {
    ...
    reconcileOutlineRebuildSelectedMaterials();
    renderOutlineRebuildModal();
    requestAnimationFrame(() => {
      els.outlineRebuildInstruction?.focus();
    });
  }
  ...
  function toggleOutlineRebuildMaterial(materialId) {
    const selected = new Set(state.rebuildModal.selectedMaterialIds || []);
    if (selected.has(materialId)) {
      selected.delete(materialId);
    } else {
      selected.add(materialId);
    }
    state.rebuildModal.selectedMaterialIds = Array.from(selected);
    clearOutlineRebuildPreview();
    renderOutlineRebuildModal();
  }
  ...
  function setCreateCourseMode(enabled) {
    const wasHidden = !!els.newCoursePanel?.classList.contains('hidden');
    if (enabled) {
  ...
  function renderOutlineRebuildModal() {
    ...
    const selectedMaterialIds = new Set(state.rebuildModal.selectedMaterialIds || []);
    ...
    if (els.outlineRebuildMaterialList) {
      els.outlineRebuildMaterialList.innerHTML = availableMaterials.map((material) => `
        <button class="selection-item${selectedMaterialIds.has(material.id) ? ' selected' : ''}" type="button" data-outline-material-id="${escapeHtml(material.id)}">
          <span class="selection-item-main">
            <span class="selection-item-title">${escapeHtml(material.fileName)}</span>
            <span class="selection-item-meta">${escapeHtml(`${subjectLabel(material.subject)} / ${statusLabels[material.status] || material.status}`)}</span>
          </span>
        </button>
